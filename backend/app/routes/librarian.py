...
async def accept_reserve_query(body: UserQueryActionModel, session: AsyncSession = Depends(get_async_session)):
...
async def accept_take_query(body: UserQueryActionModel, session: AsyncSession = Depends(get_async_session)):
	user = await users.get_user_by_id(session, body.user_id)
	match user:
		case user if user.user_type != "Librarian":
			raise HTTPException(
...
async def cancel_take_query(body: UserQueryActionModel, session: AsyncSession = Depends(get_async_session)):
	user = await users.get_user_by_id(session, body.user_id)
	match user:
		case user if user.user_type != "Librarian":
			raise HTTPException(
