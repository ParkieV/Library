...
async def search_book_by_title(session: AsyncSession, book_title: str) -> List[BookDBModel] | None:
	query = text("""
					 SELECT *
					 FROM books
...
async def get_books(session: AsyncSession, offset_db: int | None = None,
					limit_db: int | None = None) -> List[BookDBModel] | None:
